...
class ControlDynamixel(OpenRTM_aist.DataFlowComponentBase):
	...
	def onExecute(self, ec_id):
		dxl_goal_position1 = int(self._dxl_goal_position * 1024 / 90)
		...
		if dxl_comm_result != COMM_SUCCESS:
			...
			print("%s" % self._packetHandler.getRxPacketError(dxl_error))
		print("[ID:%03d] GoalPos:%03d  PresPos:%03d" % (self._DXL_ID[0], dxl_goal_position1, dxl_present_position))
		if dxl_present_position==65535:
			dxl_present_position=0
		print(dxl_goal_position1)
		if abs(dxl_goal_position1 - dxl_present_position) > self._DXL_MOVING_STATUS_THRESHOLD[0]:
			dxl_comm_result, dxl_error = self._packetHandler.write4ByteTxRx(self._portHandler, self._DXL_ID[0], self._ADDR_MX_GOAL_POSITION[0], dxl_goal_position1)
		return RTC.RTC_OK
